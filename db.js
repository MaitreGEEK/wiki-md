...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_articles_folder_position ON articles(folder_id, position);
...
export const getArticlesByFolder = db.prepare(
	"SELECT * FROM articles WHERE folder_id = ? ORDER BY position ASC, created_at DESC",
);
export const searchArticles = db.prepare(
	"SELECT * FROM articles WHERE title LIKE ? OR content LIKE ? ORDER BY created_at DESC",
);

// position auto: max(position)+1 dans le même folder (folder_id peut être NULL)
export const createArticle = db.prepare(`
  INSERT INTO articles (
    slug, title, content, folder_id, visibility, password, image, description, author_id, position
  )
  VALUES (
    ?, ?, ?, ?, ?, ?, ?, ?, ?,
    (SELECT COALESCE(MAX(position), -1) + 1 FROM articles WHERE folder_id IS ?)
  )
`);
export const updateArticle = db.prepare(
	`UPDATE articles
   SET title = ?, content = ?, folder_id = ?, visibility = ?, password = ?, image = ?, description = ?,
       updated_at = CURRENT_TIMESTAMP
   WHERE id = ?`,
);
export const deleteArticle = db.prepare("DELETE FROM articles WHERE id = ?");

export const updateArticlePosition = db.prepare(
	"UPDATE articles SET position = ? WHERE id = ?",
);
...
// tri stable: position puis nom
export const getAllFolders = db.prepare(
	"SELECT * FROM folders ORDER BY position ASC, name ASC",
);

// position auto: max(position)+1 global dossiers
export const createFolder = db.prepare(`
  INSERT INTO folders (slug, name, description, visibility, password, position)
  VALUES (
    ?, ?, ?, ?, ?,
...
export const updateFolder = db.prepare(
	`UPDATE folders
   SET name = ?, description = ?, visibility = ?, password = ?, updated_at = CURRENT_TIMESTAMP
   WHERE id = ?`,
);
export const deleteFolder = db.prepare("DELETE FROM folders WHERE id = ?");
export const updateFolderPosition = db.prepare(
